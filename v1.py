...
def add_adx(df, period=14):
    ...
    atr = tr.rolling(period).mean()
    df['ADX'] = (atr / df['Close']).rolling(period).mean() * 10000
    return df
...
def plot_candlestick(df, symbol):
    ...
    # SuperTrend 線（綠=多頭，紅=空頭）
    fig.add_trace(go.Scatter(
        x=df_plot.index,
        y=df_plot['ST_Line'],
        mode='lines',
        name='SuperTrend',
        line=dict(width=4, color='green' if df_plot['SuperTrend'].iloc[-1] == 1 else 'red')
    ))

    # VWAP、MA20、MA50
    fig.add_trace(go.Scatter(x=df_plot.index, y=df_plot['VWAP'], mode='lines', name='VWAP', line=dict(color='orange', width=2, dash='dot')))
    fig.add_trace(go.Scatter(x=df_plot.index, y=df_plot['MA20'], mode='lines', name='MA20', line=dict(color='blue', width=2)))
    fig.add_trace(go.Scatter(x=df_plot.index, y=df_plot['MA50'], mode='lines', name='MA50', line=dict(color='purple', width=2)))
...
for symbol in [s.strip() for s in symbols if s.strip()]:
    ...
    try:
        ...
        df["MA50"] = df["Close"].rolling(50).mean()    # 這行已修正！
        df = supertrend(df)
